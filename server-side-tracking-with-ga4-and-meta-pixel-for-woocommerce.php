<?php
/**
 * Plugin Name: Server-Side Tracking with GA4 and Meta Pixel for WooCommerce
 ...
 * Text Domain: server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce
 ...
 *  You may not use the name "Server-Side Tracking with GA4 and Meta Pixel for WooCommerce"
...
class GA4_S2S_Woo_Tracking
{
    ...
    public function __construct()
    {
        ...
        add_action("wp_ajax_ga4_s2s_send_test_event", [
            $this,
            "ajax_send_test_event",
        ]);
    ...
    private function init_tracking_hooks()
    {
        ...
        $enabled_events = isset($options["enabled_events"])
        ...
        if (!empty($enabled_events["add_shipping_info"])) {
            ...
            add_action("wp_ajax_track_shipping_method", [
                $this,
                "ajax_track_shipping_method",
            ]);
            add_action("wp_ajax_nopriv_track_shipping_method", [
                $this,
                "ajax_track_shipping_method",
            ]);
            if (!empty($enabled_events["add_payment_info"])) {
                add_action("wp_enqueue_scripts", [
                    $this,
                    "enqueue_checkout_tracking_script",
                ]);
            }
        }
        if (!empty($enabled_events["add_payment_info"])) {
            add_action(
                ...
                [$this, "track_add_payment_info"],
                10,
                2
            );
            add_action("wp_ajax_track_payment_method", [
                $this,
                "ajax_track_payment_method",
            ]);
            add_action("wp_ajax_nopriv_track_payment_method", [
                $this,
                "ajax_track_payment_method",
            ]);
            if (empty($enabled_events["add_shipping_info"])) {
        ...
        if (!empty($enabled_events["payment_failed"])) {
            ...
            add_action("woocommerce_payment_failed", [
                $this,
                "track_payment_failed_checkout",
            ]);
        }
    ...
    public function add_admin_menu()
    {
        ...
        add_submenu_page(
            ...
            "manage_options",
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce-logs",
            [$this, "logs_page"]
        );
    }
    ...
    public function enqueue_admin_scripts($hook)
    {
        ...
        wp_localize_script("ga4-s2s-admin", "ga4_s2s_admin", [
            ...
            "test_success" => __(
                "Test event sent successfully! Check the Event Logs to see the response.",
                "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
            ),
            "test_error" => __(
    ...
    public function sanitize_settings($input)
    {
        ...
        if (isset($input["measurement_id"])) {
            $sanitized["measurement_id"] = sanitize_text_field(
        ...
        if (isset($input["api_secret"])) {
            $sanitized["api_secret"] = sanitize_text_field(
                $input["api_secret"]
            );
        }
        ...
        if (isset($input["fb_pixel_id"])) {
            $sanitized["fb_pixel_id"] = sanitize_text_field(
                $input["fb_pixel_id"]
            );
        }

        if (isset($input["fb_access_token"])) {
            $sanitized["fb_access_token"] = sanitize_text_field(
                $input["fb_access_token"]
            );
        }

        if (isset($input["fb_test_event_code"])) {
            $sanitized["fb_test_event_code"] = sanitize_text_field(
                $input["fb_test_event_code"]
            );
        }

        // Sanitize checkboxes
        $sanitized["enable_ga4"] = isset($input["enable_ga4"]) ? "1" : "0";
        $sanitized["enable_facebook"] = isset($input["enable_facebook"])
            ? "1"
            : "0";
        ...
        $sanitized["override_currency"] = isset($input["override_currency"])
            ? "1"
            : "0";
        ...
        if (isset($input["target_currency"])) {
            ...
            $sanitized["target_currency"] = in_array(
        ...
        if (
            isset($input["enabled_events"]) &&
            is_array($input["enabled_events"])
        ) {
            ...
            foreach ($valid_events as $event) {
                $sanitized["enabled_events"][$event] = isset(
                    $input["enabled_events"][$event]
                )
                    ? "1"
                    : "0";
            }
    ...
    public function admin_page()
    {
        ...
        if (
            isset($_POST["submit"]) &&
            isset($_POST["ga4_s2s_nonce"]) &&
            wp_verify_nonce(
                sanitize_text_field(wp_unslash($_POST["ga4_s2s_nonce"])),
                "ga4_s2s_settings"
            )
        ) {
            // Process form submission
            $options = [];
            $options["measurement_id"] = isset($_POST["measurement_id"])
                ? sanitize_text_field(wp_unslash($_POST["measurement_id"]))
                : "";
            $options["api_secret"] = isset($_POST["api_secret"])
                ? sanitize_text_field(wp_unslash($_POST["api_secret"]))
                : "";
            $options["fb_pixel_id"] = isset($_POST["fb_pixel_id"])
                ? sanitize_text_field(wp_unslash($_POST["fb_pixel_id"]))
                : "";
            $options["fb_access_token"] = isset($_POST["fb_access_token"])
                ? sanitize_text_field(wp_unslash($_POST["fb_access_token"]))
                : "";
            $options["fb_test_event_code"] = isset($_POST["fb_test_event_code"])
                ? sanitize_text_field(wp_unslash($_POST["fb_test_event_code"]))
                : "";
            ...
            $options["enable_facebook"] = isset($_POST["enable_facebook"])
                ? "1"
                : "0";
            $options["enable_logging"] = isset($_POST["enable_logging"])
                ? "1"
                : "0";
            $options["debug_mode"] = isset($_POST["debug_mode"]) ? "1" : "0";
            $options["override_currency"] = isset($_POST["override_currency"])
                ? "1"
                : "0";
            $options["target_currency"] = isset($_POST["target_currency"])
                ? sanitize_text_field(wp_unslash($_POST["target_currency"]))
                : "USD";
            $options["exchange_rate"] = isset($_POST["exchange_rate"])
            ...
            $options["enabled_events"] = [
                ...
                "remove_from_cart" => isset($_POST["track_remove_from_cart"])
                    ? "1"
                    : "0",
                "begin_checkout" => isset($_POST["track_begin_checkout"])
                    ? "1"
                    : "0",
                "add_shipping_info" => isset($_POST["track_add_shipping_info"])
                    ? "1"
                    : "0",
                "add_payment_info" => isset($_POST["track_add_payment_info"])
                    ? "1"
                    : "0",
                "purchase" => isset($_POST["track_purchase"]) ? "1" : "0",
                "payment_failed" => isset($_POST["track_payment_failed"])
                    ? "1"
                    : "0",
...
<div class="wrap">
	<h1><?php echo esc_html__(
     "GA4 & Facebook Pixel Server-Side Tracking Settings",
     "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
 ); ?></h1>
	...
	<form method="post" action="">
      ...
      "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
				...
				<td>
					...
					<label style="margin-right: 20px;">
						<input type="checkbox" name="enable_ga4" value="1"
							<?php checked(
           isset($options["enable_ga4"]) && $options["enable_ga4"] === "1"
       ); ?> />
						<?php echo esc_html__(
          "Google Analytics 4",
          "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
					...
					<label>
						<input type="checkbox" name="enable_facebook" value="1"
							<?php checked(
           isset($options["enable_facebook"]) &&
               $options["enable_facebook"] === "1"
       ); ?> />
						<?php echo esc_html__(
          "Facebook Pixel",
          "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
		...
		<div id="ga4-configuration-section" style="<?php echo !isset(
      $options["enable_ga4"]
  ) || $options["enable_ga4"] !== "1"
      ? "display:none;"
      : ""; ?>">
			<h2><?php echo esc_html__(
       ...
       "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
			...
			<table class="form-table">
				<tr>
					<th scope="row"><label for="measurement_id"><?php echo esc_html__(
         "Measurement ID",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></label></th>
					<td>
           ...
           "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
            ...
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
				...
				<tr>
					<th scope="row"><label for="api_secret"><?php echo esc_html__(
         "API Secret",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></label></th>
					<td>
           ...
           "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
            ...
            "How to get your GA4 credentials:",
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
        ); ?></strong>
							</p>
		...
		<div id="facebook-configuration-section" style="<?php echo !isset(
      $options["enable_facebook"]
  ) || $options["enable_facebook"] !== "1"
      ? "display:none;"
      : ""; ?>">
			<h2><?php echo esc_html__(
       ...
       "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
			...
			<table class="form-table">
				<tr>
					<th scope="row"><label for="fb_pixel_id"><?php echo esc_html__(
         "Pixel ID",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></label></th>
					<td>
						...
						<p class="description">
							<?php echo esc_html__(
           "Your Facebook Pixel ID (16 digits)",
           "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
       ); ?><br>
            ...
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
				...
				<tr>
					<th scope="row"><label for="fb_access_token"><?php echo esc_html__(
         "Access Token",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></label></th>
					<td>
           ...
           "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
            ...
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
				...
				<tr>
					<th scope="row"><label for="fb_test_event_code"><?php echo esc_html__(
         "Test Event Code",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></label></th>
					<td>
						...
						<p class="description">
							<?php echo esc_html__(
           "Optional: Test event code from Facebook Events Manager for debugging",
           "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
       ); ?>
						</p>
					</td>
				...
				<tr>
					...
					<td>
						<div style="background: #f5f5f5; padding: 15px; border-radius: 5px; margin-top: 10px;">
							<p style="margin: 0 0 10px 0; color: #666;">
								<strong><?php echo esc_html__(
            "How to get your Facebook Pixel credentials:",
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
        ); ?></strong>
							</p>
							<ol style="margin: 0 0 10px 0; padding-left: 20px; color: #666;">
            ...
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
								...
								<li><?php echo esc_html__(
            "(Optional) Enable Test Events and get Test Event Code",
            "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
        ); ?></li>
							</ol>
							<p style="margin: 0; color: #666;">
     ...
     ?>
			...
			<tr class="currency-override-settings" style="<?php echo empty(
       $options["override_currency"]
   )
       ? "display:none;"
       : ""; ?>">
				...
				<td>
					<select name="target_currency" id="target_currency">
						<option value="USD" <?php selected(
          $options["target_currency"] ?? "USD",
          "USD"
          ...
          "EUR"
						...
						<option value="GBP" <?php selected(
          $options["target_currency"] ?? "USD",
          "GBP"
      ); ?>>GBP - British Pound</option>
						<option value="JPY" <?php selected(
          $options["target_currency"] ?? "USD",
          "JPY"
      ); ?>>JPY - Japanese Yen</option>
						<option value="AUD" <?php selected(
          $options["target_currency"] ?? "USD",
          "AUD"
      ); ?>>AUD - Australian Dollar</option>
						<option value="CAD" <?php selected(
          $options["target_currency"] ?? "USD",
          "CAD"
      ...
      ); ?> = <span id="exchange-preview"><?php echo esc_html(
     $options["exchange_rate"] ?? "1.0"
 ); ?></span> <span id="target-currency-preview"><?php echo esc_html(
    $options["target_currency"] ?? "USD"
); ?></span><br>
						<?php 
      printf(
          /* translators: %s: The WooCommerce store currency code (e.g., EUR, GBP) */
          esc_html__(
              "Example: If 1 %s = 1.2 USD, enter 1.2",
              "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
          ),
      ...
      "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
		...
		<table class="form-table">
         ...
         "Enable this to track all events sent to GA4 in the Event Logs",
         "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
     ); ?></p>
				</td>
			</tr>
	...
	<p style="text-align: center; color: #666; font-size: 12px; margin-top: 20px;">
      ...
      sprintf(
          ...
          esc_html__(
              'Server-Side Tracking with GA4 and Meta Pixel for WooCommerce v%1$s | Created by %2$s',
              "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
          ),
          "1.1.0",
    ...
    public function logs_page()
    {
        ...
        if ($logs_query->have_posts()) {
            while ($logs_query->have_posts()) {
                ...
                $log = new stdClass();
                $log->id = $post_id;
                $log->timestamp = get_post_meta(
                    $post_id,
                    "_ga4_timestamp",
                    true
                );
                $log->event_name = get_post_meta(
                    $post_id,
                    "_ga4_event_name",
                    true
                );
                $log->event_params = get_post_meta(
                    $post_id,
                    "_ga4_event_params",
                    true
                );
                $log->response_code = get_post_meta(
                    $post_id,
                    "_ga4_response_code",
                    true
                );
                $log->response_body = get_post_meta(
                    $post_id,
                    "_ga4_response_body",
                    true
                );
                $log->debug_info = get_post_meta(
                    $post_id,
                    "_ga4_debug_info",
                    true
                );
    ...
    "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
				...
				<?php else: ?>
					<?php foreach ($logs as $log): ?>
						<tr data-log-id="<?php echo esc_attr($log->id); ?>">
							<td><input type="checkbox" class="log-checkbox" value="<?php echo esc_attr(
           $log->id
       ); ?>" /></td>
          ...
          ?>
           ...
           ?>
							...
							<?php endif; ?>
							<td>
								<button class="button button-small delete-single-log" data-log-id="<?php echo esc_attr(
            $log->id
        ); ?>">
									<?php echo esc_html__(
             "Delete",
...
<?php
    ...
    public function ajax_delete_logs()
    {
        ...
        $logs_to_delete =
            isset($_POST["logs"]) && is_array($_POST["logs"])
                ? array_map("intval", $_POST["logs"])
                : [];
    ...
    public function ajax_send_test_event()
    {
        ...
        $platform = isset($_POST["platform"])
            ? sanitize_text_field(wp_unslash($_POST["platform"]))
            : "both";
        ...
        if (empty($messages)) {
            wp_send_json_error(
                __(
                    "No platforms are enabled for testing.",
                    "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
                )
            );
        } else {
            wp_send_json_success([
                "message" =>
                    ...
                    ". " .
                    __(
                        "Check the Event Logs to verify.",
                        "server-side-tracking-with-ga4-and-meta-pixel-for-woocommerce"
                    ),
            ]);
        }
    ...
    private function convert_price($price)
    {
        ...
        if (
            !empty($options["override_currency"]) &&
            !empty($options["exchange_rate"])
        ) {
    ...
    private function log_event(
        ...
        if (!empty($options["debug_mode"])) {
            $debug_info = [
                ...
                "response_headers" => !is_wp_error($response)
    ...
    private function send_ga4_event(
        $event_name,
        $params = [],
        $user_id = null,
        $client_id = null
        ...
        $measurement_id = isset($options["measurement_id"])
            ? $options["measurement_id"]
            : "";
        $api_secret = isset($options["api_secret"])
            ? $options["api_secret"]
            : "";
        ...
        if ($user_id) {
            ...
            if ($user) {
                ...
                if (class_exists("WC_Customer")) {
                    ...
                    $params["user_properties"]["total_spent"] = round(
        ...
        if (isset($_SERVER["HTTP_REFERER"])) {
            $params["page_referrer"] = esc_url_raw(
        ...
        $uri = isset($_SERVER["REQUEST_URI"])
            ...
            : "";
        $params["page_location"] = esc_url_raw(
            $protocol . "://" . $host . $uri
        );

        // Add user agent data
        if (isset($_SERVER["HTTP_USER_AGENT"])) {
            $params["user_agent"] = sanitize_text_field(
    ...
    private function send_facebook_event(
        $event_name,
        $params = [],
        $user_id = null
    ) {
        ...
        $pixel_id = isset($options["fb_pixel_id"])
            ? $options["fb_pixel_id"]
            : "";
        $access_token = isset($options["fb_access_token"])
            ? $options["fb_access_token"]
            : "";
        ...
        if (!$pixel_id || !$access_token) {
            if (!empty($options["debug_mode"])) {
                $this->log_event(
                    ...
                    new WP_Error(
                        "missing_credentials",
                        "Missing Facebook Pixel credentials"
                    ),
                    [
        ...
        $fb_event_name = isset($fb_event_map[$event_name])
            ? $fb_event_map[$event_name]
            : $event_name;
        ...
        // Get user agent
        if (isset($_SERVER["HTTP_USER_AGENT"])) {
            $user_data["client_user_agent"] = sanitize_text_field(
                wp_unslash($_SERVER["HTTP_USER_AGENT"])
            );
        }
        ...
        if ($user_id) {
            ...
            $user_data["fn"] = hash(
                "sha256",
                strtolower($current_user->first_name)
            );
            $user_data["ln"] = hash(
                "sha256",
                strtolower($current_user->last_name)
            );
        ...
        if (function_exists("WC") && WC()->session) {
            ...
            if ($customer && is_array($customer)) {
                ...
                if (
                    empty($user_data["ph"]) &&
                    !empty($customer["billing_phone"])
                ) {
                    $phone_clean = preg_replace(
                        "/[^0-9]/",
                        "",
                ...
                if (empty($user_data["fn"])) {
                    if (!empty($customer["billing_first_name"])) {
                        $user_data["fn"] = hash(
                            "sha256",
                            strtolower($customer["billing_first_name"])
                        );
                ...
                if (empty($user_data["ln"])) {
                    if (!empty($customer["billing_last_name"])) {
                        $user_data["ln"] = hash(
                            "sha256",
                            strtolower($customer["billing_last_name"])
                        );
                ...
                if (empty($user_data["ct"])) {
                    if (!empty($customer["billing_city"])) {
                        $user_data["ct"] = hash(
                            "sha256",
                            strtolower($customer["billing_city"])
                        );
                ...
                if (empty($user_data["st"])) {
                    if (!empty($customer["billing_state"])) {
                        $user_data["st"] = hash(
                            "sha256",
                            strtolower($customer["billing_state"])
                        );
                ...
                if (empty($user_data["country"])) {
                    if (!empty($customer["billing_country"])) {
                        $user_data["country"] = hash(
                            "sha256",
                            strtolower($customer["billing_country"])
                        );
                    } elseif (!empty($customer["country"])) {
                        $user_data["country"] = hash(
                            "sha256",
                            strtolower($customer["country"])
                        );
                    }
                }
            }
        ...
        if (isset($_COOKIE["_fbc"])) {
            $user_data["fbc"] = sanitize_text_field(
        ...
        if (isset($_COOKIE["_fbp"])) {
            $user_data["fbp"] = sanitize_text_field(
                wp_unslash($_COOKIE["_fbp"])
            );
        }
            ...
            foreach ($params["items"] as $item) {
                ...
                $contents[] = [
                    ...
                    "quantity" => isset($item["quantity"])
                        ? (int) $item["quantity"]
                        : 1,
                    "item_price" => isset($item["price"])
                        ? (float) $item["price"]
                        : 0,
                ];
                $num_items += isset($item["quantity"])
                    ? (int) $item["quantity"]
                    : 1;
            }
        ...
        $test_event_code = isset($options["fb_test_event_code"])
            ? trim($options["fb_test_event_code"])
            : "";
    ...
    public function track_add_shipping_info($post_data)
    {
        ...
        if (
            ...
            sanitize_text_field($_SESSION[$session_key]) === $transaction_id
        ) {
            return;
        }
        ...
        if (!empty($packages)) {
            foreach ($packages as $package) {
                if (isset($package["rates"][$shipping_method])) {
                    $shipping_method_title = $package["rates"][
                        $shipping_method
                    ]->get_label();
                    break;
                }
        ...
        if (isset($posted["shipping_country"])) {
            $shipping_data["shipping_country"] = sanitize_text_field(
                $posted["shipping_country"]
            );
        }
        if (isset($posted["shipping_state"])) {
            $shipping_data["shipping_state"] = sanitize_text_field(
                $posted["shipping_state"]
            );
        }
        if (isset($posted["shipping_city"])) {
            $shipping_data["shipping_city"] = sanitize_text_field(
                $posted["shipping_city"]
            );
        }
        if (isset($posted["shipping_postcode"])) {
            $shipping_data["shipping_postcode"] = sanitize_text_field(
        ...
        $params = array_merge(
            [
                ...
                "currency" => $this->get_currency(),
                "value" => round(
                    $total_value + $this->convert_price($shipping_total),
                    2
                ),
                "shipping_tier" => $shipping_method_title ?: $shipping_method,
                "items" => $items,
    ...
    public function ajax_track_shipping_method()
    {
        ...
        if (
            ...
            $shipping_methods = array_map(
                "sanitize_text_field",
                wp_unslash($_POST["shipping_method"])
            );
        ...
        $params = [
            ...
            "currency" => $this->get_currency(),
            "value" => round(
                $total_value + $this->convert_price($shipping_cost),
                2
            ),
            "shipping_tier" => $shipping_method_title ?: $shipping_method,
    ...
    public function track_add_payment_info($data, $errors)
    {
        ...
        if (
            isset($_SESSION[$session_key]) &&
            sanitize_text_field($_SESSION[$session_key]) ===
    ...
    public function enqueue_checkout_tracking_script()
    {
        ...
        $options = get_option($this->options_name, []);
        $track_shipping = !empty(
            $options["enabled_events"]["add_shipping_info"]
        );
    ...
    public function track_payment_failed_checkout()
    {
        ...
        if (
            ...
            if (
                ...
                !wp_verify_nonce(
                    sanitize_text_field(
                        wp_unslash($_REQUEST["woocommerce-pay-nonce"])
                    ),
                    "woocommerce-pay"
                )
            ) {
        ...
        $params = [
            ...
            "value" => round($total_value, 2),
            "items" => $items,
            "payment_type" => isset($_POST["payment_method"])
                ? sanitize_text_field(wp_unslash($_POST["payment_method"]))
                : "unknown",
        ];
